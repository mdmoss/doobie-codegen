...
object TestDoubleFk_3 extends TestDoubleFk_3 {
  ...
  case class Shape(id: Long, notpk: Long)

  object Shape {
    def NoDefaults(id: Long, notpk: Long): Shape = Shape(id, notpk)
  }
...
trait TestDoubleFk_3 {
  ...
  def create(id: Long, notpk: Long): ConnectionIO[Row] = {
    create(Shape(id, notpk))
  }

  def createVoid(id: Long, notpk: Long): ConnectionIO[Unit] = {
  ...
  private[gen] def multigetInnerBase(id: Option[Seq[mdmoss.doobiegen.db.gen.TestDoubleFk_3.Id]]): Query0[Row] = {
    (sql"""
      SELECT """ ++ Row.ColumnsFragment ++ sql"""
      FROM test_double_fk_3
      WHERE (${id.isEmpty} OR test_double_fk_3.id = ANY(${{id}.toSeq.flatten.map(_.value).toArray}))
    """).query[Row]
  }
