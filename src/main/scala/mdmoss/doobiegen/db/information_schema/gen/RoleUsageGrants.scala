...
object RoleUsageGrants extends RoleUsageGrants {
...
trait RoleUsageGrants {
  import RoleUsageGrants._
  ...
  private[gen] def insertMany(values: List[mdmoss.doobiegen.db.information_schema.gen.RoleUsageGrants.Shape]): Update[Shape] = {
    val sql = "INSERT INTO information_schema.role_usage_grants (grantor, grantee, object_catalog, object_schema, object_name, object_type, privilege_type, is_grantable) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
    Update[Shape](sql)
  }

  private[gen] def createManyP(values: List[mdmoss.doobiegen.db.information_schema.gen.RoleUsageGrants.Shape]): scalaz.stream.Process[ConnectionIO, Row] = {
  ...
  def createMany(values: List[mdmoss.doobiegen.db.information_schema.gen.RoleUsageGrants.Shape]): ConnectionIO[List[Row]] = {
    if (values.nonEmpty) createManyP(values).runLog.map(_.toList) else List.empty.point[ConnectionIO]
  }

  def createManyVoid(values: List[mdmoss.doobiegen.db.information_schema.gen.RoleUsageGrants.Shape]): ConnectionIO[Unit] = {
    if (values.nonEmpty) insertMany(values).updateMany[List](values).map(_ => ()) else ().point[ConnectionIO]
  }

  def create(shape: mdmoss.doobiegen.db.information_schema.gen.RoleUsageGrants.Shape): ConnectionIO[Row] = {
    createMany(shape :: Nil).map(_.head)
  }

  def createVoid(shape: mdmoss.doobiegen.db.information_schema.gen.RoleUsageGrants.Shape): ConnectionIO[Unit] = {
