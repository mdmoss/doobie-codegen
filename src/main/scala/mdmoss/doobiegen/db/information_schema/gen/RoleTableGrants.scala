...
object RoleTableGrants extends RoleTableGrants {
...
trait RoleTableGrants {
  import RoleTableGrants._
  ...
  private[gen] def insertMany(values: List[mdmoss.doobiegen.db.information_schema.gen.RoleTableGrants.Shape]): Update[Shape] = {
    val sql = "INSERT INTO information_schema.role_table_grants (grantor, grantee, table_catalog, table_schema, table_name, privilege_type, is_grantable, with_hierarchy) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
    Update[Shape](sql)
  }

  private[gen] def createManyP(values: List[mdmoss.doobiegen.db.information_schema.gen.RoleTableGrants.Shape]): scalaz.stream.Process[ConnectionIO, Row] = {
  ...
  def createMany(values: List[mdmoss.doobiegen.db.information_schema.gen.RoleTableGrants.Shape]): ConnectionIO[List[Row]] = {
    if (values.nonEmpty) createManyP(values).runLog.map(_.toList) else List.empty.point[ConnectionIO]
  }

  def createManyVoid(values: List[mdmoss.doobiegen.db.information_schema.gen.RoleTableGrants.Shape]): ConnectionIO[Unit] = {
    if (values.nonEmpty) insertMany(values).updateMany[List](values).map(_ => ()) else ().point[ConnectionIO]
  }

  def create(shape: mdmoss.doobiegen.db.information_schema.gen.RoleTableGrants.Shape): ConnectionIO[Row] = {
    createMany(shape :: Nil).map(_.head)
  }

  def createVoid(shape: mdmoss.doobiegen.db.information_schema.gen.RoleTableGrants.Shape): ConnectionIO[Unit] = {
