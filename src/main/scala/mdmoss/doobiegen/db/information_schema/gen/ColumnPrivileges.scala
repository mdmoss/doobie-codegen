...
object ColumnPrivileges extends ColumnPrivileges {
...
trait ColumnPrivileges {
  import ColumnPrivileges._
  ...
  private[gen] def insertMany(values: List[mdmoss.doobiegen.db.information_schema.gen.ColumnPrivileges.Shape]): Update[Shape] = {
    val sql = "INSERT INTO information_schema.column_privileges (grantor, grantee, table_catalog, table_schema, table_name, column_name, privilege_type, is_grantable) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
    Update[Shape](sql)
  }

  private[gen] def createManyP(values: List[mdmoss.doobiegen.db.information_schema.gen.ColumnPrivileges.Shape]): scalaz.stream.Process[ConnectionIO, Row] = {
  ...
  def createMany(values: List[mdmoss.doobiegen.db.information_schema.gen.ColumnPrivileges.Shape]): ConnectionIO[List[Row]] = {
    if (values.nonEmpty) createManyP(values).runLog.map(_.toList) else List.empty.point[ConnectionIO]
  }

  def createManyVoid(values: List[mdmoss.doobiegen.db.information_schema.gen.ColumnPrivileges.Shape]): ConnectionIO[Unit] = {
    if (values.nonEmpty) insertMany(values).updateMany[List](values).map(_ => ()) else ().point[ConnectionIO]
  }

  def create(shape: mdmoss.doobiegen.db.information_schema.gen.ColumnPrivileges.Shape): ConnectionIO[Row] = {
    createMany(shape :: Nil).map(_.head)
  }

  def createVoid(shape: mdmoss.doobiegen.db.information_schema.gen.ColumnPrivileges.Shape): ConnectionIO[Unit] = {
