...
object ApplicableRoles extends ApplicableRoles {
...
trait ApplicableRoles {
  import ApplicableRoles._
  ...
  private[gen] def insertMany(values: List[mdmoss.doobiegen.db.information_schema.gen.ApplicableRoles.Shape]): Update[Shape] = {
    val sql = "INSERT INTO information_schema.applicable_roles (grantee, role_name, is_grantable) VALUES (?, ?, ?)"
    Update[Shape](sql)
  }

  private[gen] def createManyP(values: List[mdmoss.doobiegen.db.information_schema.gen.ApplicableRoles.Shape]): scalaz.stream.Process[ConnectionIO, Row] = {
    insertMany(values).updateManyWithGeneratedKeys[Row]("grantee", "role_name", "is_grantable")(values)
  }

  def createMany(values: List[mdmoss.doobiegen.db.information_schema.gen.ApplicableRoles.Shape]): ConnectionIO[List[Row]] = {
    if (values.nonEmpty) createManyP(values).runLog.map(_.toList) else List.empty.point[ConnectionIO]
  }

  def createManyVoid(values: List[mdmoss.doobiegen.db.information_schema.gen.ApplicableRoles.Shape]): ConnectionIO[Unit] = {
    if (values.nonEmpty) insertMany(values).updateMany[List](values).map(_ => ()) else ().point[ConnectionIO]
  }

  def create(shape: mdmoss.doobiegen.db.information_schema.gen.ApplicableRoles.Shape): ConnectionIO[Row] = {
    createMany(shape :: Nil).map(_.head)
  }

  def createVoid(shape: mdmoss.doobiegen.db.information_schema.gen.ApplicableRoles.Shape): ConnectionIO[Unit] = {
