...
object PgStatioSysSequences extends PgStatioSysSequences {
...
trait PgStatioSysSequences {
  import PgStatioSysSequences._
  ...
  private[gen] def insertMany(values: List[mdmoss.doobiegen.db.pg_catalog.gen.PgStatioSysSequences.Shape]): Update[Shape] = {
    val sql = "INSERT INTO pg_catalog.pg_statio_sys_sequences (blks_read, blks_hit) VALUES (?, ?)"
    Update[Shape](sql)
  }

  private[gen] def createManyP(values: List[mdmoss.doobiegen.db.pg_catalog.gen.PgStatioSysSequences.Shape]): scalaz.stream.Process[ConnectionIO, Row] = {
    insertMany(values).updateManyWithGeneratedKeys[Row]("blks_read", "blks_hit")(values)
  }

  def createMany(values: List[mdmoss.doobiegen.db.pg_catalog.gen.PgStatioSysSequences.Shape]): ConnectionIO[List[Row]] = {
    if (values.nonEmpty) createManyP(values).runLog.map(_.toList) else List.empty.point[ConnectionIO]
  }

  def createManyVoid(values: List[mdmoss.doobiegen.db.pg_catalog.gen.PgStatioSysSequences.Shape]): ConnectionIO[Unit] = {
    if (values.nonEmpty) insertMany(values).updateMany[List](values).map(_ => ()) else ().point[ConnectionIO]
  }

  def create(shape: mdmoss.doobiegen.db.pg_catalog.gen.PgStatioSysSequences.Shape): ConnectionIO[Row] = {
    createMany(shape :: Nil).map(_.head)
  }

  def createVoid(shape: mdmoss.doobiegen.db.pg_catalog.gen.PgStatioSysSequences.Shape): ConnectionIO[Unit] = {
