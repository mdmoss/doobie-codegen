...
object PgStatUserIndexes extends PgStatUserIndexes {
...
trait PgStatUserIndexes {
  import PgStatUserIndexes._
  ...
  private[gen] def insertMany(values: List[mdmoss.doobiegen.db.pg_catalog.gen.PgStatUserIndexes.Shape]): Update[Shape] = {
    val sql = "INSERT INTO pg_catalog.pg_stat_user_indexes (idx_scan, idx_tup_read, idx_tup_fetch) VALUES (?, ?, ?)"
    Update[Shape](sql)
  }

  private[gen] def createManyP(values: List[mdmoss.doobiegen.db.pg_catalog.gen.PgStatUserIndexes.Shape]): scalaz.stream.Process[ConnectionIO, Row] = {
    insertMany(values).updateManyWithGeneratedKeys[Row]("idx_scan", "idx_tup_read", "idx_tup_fetch")(values)
  }

  def createMany(values: List[mdmoss.doobiegen.db.pg_catalog.gen.PgStatUserIndexes.Shape]): ConnectionIO[List[Row]] = {
    if (values.nonEmpty) createManyP(values).runLog.map(_.toList) else List.empty.point[ConnectionIO]
  }

  def createManyVoid(values: List[mdmoss.doobiegen.db.pg_catalog.gen.PgStatUserIndexes.Shape]): ConnectionIO[Unit] = {
    if (values.nonEmpty) insertMany(values).updateMany[List](values).map(_ => ()) else ().point[ConnectionIO]
  }

  def create(shape: mdmoss.doobiegen.db.pg_catalog.gen.PgStatUserIndexes.Shape): ConnectionIO[Row] = {
    createMany(shape :: Nil).map(_.head)
  }

  def createVoid(shape: mdmoss.doobiegen.db.pg_catalog.gen.PgStatUserIndexes.Shape): ConnectionIO[Unit] = {
