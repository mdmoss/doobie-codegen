...
object PgStatioUserTables extends PgStatioUserTables {
...
trait PgStatioUserTables {
  import PgStatioUserTables._
  ...
  private[gen] def insertMany(values: List[mdmoss.doobiegen.db.pg_catalog.gen.PgStatioUserTables.Shape]): Update[Shape] = {
    val sql = "INSERT INTO pg_catalog.pg_statio_user_tables (heap_blks_read, heap_blks_hit, idx_blks_read, idx_blks_hit, toast_blks_read, toast_blks_hit, tidx_blks_read, tidx_blks_hit) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
    Update[Shape](sql)
  }

  private[gen] def createManyP(values: List[mdmoss.doobiegen.db.pg_catalog.gen.PgStatioUserTables.Shape]): scalaz.stream.Process[ConnectionIO, Row] = {
  ...
  def createMany(values: List[mdmoss.doobiegen.db.pg_catalog.gen.PgStatioUserTables.Shape]): ConnectionIO[List[Row]] = {
    if (values.nonEmpty) createManyP(values).runLog.map(_.toList) else List.empty.point[ConnectionIO]
  }

  def createManyVoid(values: List[mdmoss.doobiegen.db.pg_catalog.gen.PgStatioUserTables.Shape]): ConnectionIO[Unit] = {
    if (values.nonEmpty) insertMany(values).updateMany[List](values).map(_ => ()) else ().point[ConnectionIO]
  }

  def create(shape: mdmoss.doobiegen.db.pg_catalog.gen.PgStatioUserTables.Shape): ConnectionIO[Row] = {
    createMany(shape :: Nil).map(_.head)
  }

  def createVoid(shape: mdmoss.doobiegen.db.pg_catalog.gen.PgStatioUserTables.Shape): ConnectionIO[Unit] = {
